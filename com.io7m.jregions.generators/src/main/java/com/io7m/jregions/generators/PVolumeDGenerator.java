...
public final class PVolumeDGenerator<S> implements Generator<PVolumeD<S>>
{
  ...
  public static <S> PVolumeDGenerator<S> create()
  {
    return new PVolumeDGenerator<>(new DoubleGenerator(0.0, 10000.0));
  }

  @Override
  public PVolumeD<S> next()
  {
    ...
    final double z_min = order.get(0).doubleValue();
    final double z_max = order.get(1).doubleValue();

    return PVolumeD.of(x_min, x_max, y_min, y_max, z_min, z_max);
  }
}
